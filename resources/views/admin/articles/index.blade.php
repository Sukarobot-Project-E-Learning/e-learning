...
@section('content')
<div class="container px-6 mx-auto" x-data="articleTable()">
    ...
    <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-orange-100 dark:border-gray-700">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300ms="filterTable()" placeholder="Cari judul, kategori, penulis..." class="w-full pl-10 pr-4 py-2.5 text-sm border-2 border-orange-200 rounded-lg focus:border-orange-500 focus:ring-4 focus:ring-orange-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all duration-200">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4" :style="'animation-delay:' + (index * 50) + 'ms'">
                <div class="flex items-start gap-3">
                    <template x-if="article.image">
                        <img :src="article.image" class="w-16 h-12 rounded-lg object-cover border-2 border-orange-200" @click="window.open(article.image, '_blank')">
                    </template>
                    <template x-if="!article.image">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-800 dark:text-white truncate" x-html="highlightText(article.title)"></h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400" x-html="highlightText(article.category)"></p>
                        <p class="text-xs text-gray-400" x-text="article.date"></p>
                    </div>
                    <span :class="article.status === 'Aktif' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'" class="px-2 py-1 text-xs font-bold rounded-full whitespace-nowrap" x-text="article.status"></span>
                </div>
                ...
                <tbody class="divide-y divide-orange-100 dark:divide-gray-700">
                    <template x-for="(article, index) in articles" :key="article.id">
                        <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150" :style="'animation-delay:' + (index * 50) + 'ms'">
                            <td class="px-6 py-4"><div class="font-semibold text-gray-800 dark:text-white" x-html="highlightText(article.title)"></div></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-html="highlightText(article.category)"></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-text="article.date"></td>
                            <td class="px-6 py-4">
                                <template x-if="article.image"><img :src="article.image" class="w-16 h-12 rounded-lg object-cover border-2 border-orange-200 hover:border-orange-400 transition-all cursor-pointer hover:scale-110" @click="window.open(article.image, '_blank')"></template>
                                <template x-if="!article.image"><div class="w-16 h-12 rounded-lg bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center"><svg class="w-6 h-6 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div></template>
                            </td>
