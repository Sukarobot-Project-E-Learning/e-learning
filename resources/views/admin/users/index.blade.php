        ...
        <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-orange-100 dark:border-gray-700">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm text-gray-500 dark:text-gray-400">
                <div class="flex items-center gap-1">
                    ...
                    <span>dari</span><span class="font-semibold text-orange-600" x-text="total"></span><span>user</span>
                </div>
        ...
        <div class="block lg:hidden space-y-3 mb-6">
            <template x-for="(user, index) in users" :key="user.id">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4"
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <template x-if="!user.avatar">
                            <div
                                class="w-12 h-12 rounded-full bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-gray-800 dark:text-white truncate"
                                x-html="highlightText(user.name)"></h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 truncate"
                                x-html="highlightText(user.email || '-')"></p>
                            <p class="text-sm text-gray-500 dark:text-gray-400" x-html="highlightText(user.phone || '-')">
                            </p>
                        </div>
                        <span
            ...
            <template x-if="users.length === 0">
                <div
                    ...
                    <a href="{{ route('admin.users.create') }}"
                        class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Tambah user baru</a>
                </div>
            </template>
        </div>
                            ...
                            <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150"
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-gray-800 dark:text-white"
                                        x-html="highlightText(user.name)"></div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                    x-html="highlightText(user.email || '-')"></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                ...
                                <td class="px-6 py-4"><span
                                        ...
                                        x-text="user.is_active ? 'Aktif' : 'Tidak Aktif'"></span></td>
                                <td class="px-6 py-4">
                        ...
                        <template x-if="users.length === 0">
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500"><svg
                                    ...
                                    <p class="text-lg font-medium">Tidak ada user ditemukan</p><a
    ...
    <script>
        function userTable() {
            return {
                ...
                deleteUser(id) {
                    Swal.fire({ title: "Hapus user?", text: "Data tidak dapat dikembalikan!", icon: "warning", showCancelButton: true, confirmButtonColor: "#f97316", cancelButtonColor: "#6b7280", confirmButtonText: "Ya, hapus!" }).then((result) => {
                        if (result.isConfirmed) {
                            fetch(`/admin/users/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } })
