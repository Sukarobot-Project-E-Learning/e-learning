...
@section('content')
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                Belum ada user. <button @click="$dispatch('open-modal', { type: 'create' })" class="text-purple-600 hover:text-purple-800 dark:text-purple-400">Tambah user pertama</button>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div x-data="{
        ...
        submitForm() {
            const url = this.type === 'create' ? '{{ route("admin.users.store") }}' : `{{ url("admin/users") }}/${this.userId}`;
    ...
    @click.self="open = false">
        <div class="relative w-full max-w-md p-6 mx-4 bg-white rounded-lg shadow-xl dark:bg-gray-800" @click.stop>
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white" x-text="type === 'create' ? 'Tambah User' : 'Edit User'"></h3>
