...
<div class="container px-6 mx-auto overflow-x-hidden max-w-full" x-data="promoTable()">
    <div class="my-6 flex items-center justify-between">
        ...
        <a href="{{ route('admin.promos.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 transition-all duration-200">
    ...
    <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-orange-100 dark:border-gray-700">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300ms="filterTable()" placeholder="Cari judul promo..." class="w-full pl-10 pr-4 py-2.5 text-sm border-2 border-orange-200 rounded-lg focus:border-orange-500 focus:ring-4 focus:ring-orange-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all duration-200">
    ...
    <div class="block lg:hidden space-y-3 mb-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-800 dark:text-white truncate" x-html="highlightText(promo.title)"></h3>
                    </div>
                    <span :class="promo.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'" class="px-2 py-1 text-xs font-bold rounded-full" x-text="promo.status"></span>
                </div>
                <div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-2">
                    <a :href="'/admin/promos/' + promo.id + '/edit'" class="px-3 py-1.5 text-sm font-medium text-orange-600 hover:bg-orange-50 dark:hover:bg-gray-700 rounded-lg">Edit</a>
                    <button @click="deletePromo(promo.id)" class="px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-gray-700 rounded-lg">Hapus</button>
                </div>
            </div>
        </template>
        <template x-if="promos.length === 0">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
                ...
                <a href="{{ route('admin.promos.create') }}" class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Tambah promo baru</a>
            </div>
        </template>
    </div>
                ...
                <tbody class="divide-y divide-orange-100 dark:divide-gray-700">
                    <template x-for="(promo, index) in promos" :key="promo.id">
                        <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150">
                            <td class="px-6 py-4"><div class="font-semibold text-gray-800 dark:text-white" x-html="highlightText(promo.title)"></div></td>
                            <td class="px-6 py-4">
                                <template x-if="promo.poster"><img :src="promo.poster" class="w-16 h-12 rounded object-cover border border-gray-300 cursor-pointer hover:scale-110 transition-all" @click="window.open(promo.poster, '_blank')"></template>
                                <template x-if="!promo.poster"><div class="w-16 h-12 rounded bg-gray-200 dark:bg-gray-600 flex items-center justify-center"><svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div></template>
                            </td>
                            <td class="px-6 py-4">
                                <template x-if="promo.carousel"><img :src="promo.carousel" class="w-16 h-12 rounded object-cover border border-gray-300 cursor-pointer hover:scale-110 transition-all" @click="window.open(promo.carousel, '_blank')"></template>
                                <template x-if="!promo.carousel"><div class="w-16 h-12 rounded bg-gray-200 dark:bg-gray-600 flex items-center justify-center"><svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div></template>
                            </td>
                            <td class="px-6 py-4"><span :class="promo.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'" class="px-3 py-1 text-xs font-bold rounded-full" x-text="promo.status"></span></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <a :href="'/admin/promos/' + promo.id + '/edit'" class="p-2 text-orange-600 hover:bg-orange-100 rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                    <button @click="deletePromo(promo.id)" class="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                </div>
                            </td>
                        </tr>
                    </template>
                    <template x-if="promos.length === 0">
                        <tr><td colspan="5" class="px-6 py-12 text-center text-gray-500"><p class="text-lg font-medium">Tidak ada promo ditemukan</p><a href="{{ route('admin.promos.create') }}" class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Tambah promo baru</a></td></tr>
                    </template>
                </tbody>
            </table>
        </div>
