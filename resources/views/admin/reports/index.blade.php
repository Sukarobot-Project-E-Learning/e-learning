...
@section('content')
<div class="container px-6 mx-auto overflow-x-hidden max-w-full" x-data="reportTable()">
    <div class="my-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Laporan</h2>
        <a href="{{ route('admin.reports.export') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 focus:ring-4 focus:ring-orange-300 transition-all duration-200 transform hover:scale-105">
    ...
    <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-orange-100 dark:border-gray-700">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300ms="filterTable()" placeholder="Cari judul program..." class="w-full pl-10 pr-4 py-2.5 text-sm border-2 border-orange-200 rounded-lg focus:border-orange-500 focus:ring-4 focus:ring-orange-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all duration-200">
        ...
        <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm text-gray-500 dark:text-gray-400">
            <div class="flex items-center gap-1">
                ...
                <span>dari</span><span class="font-semibold text-orange-600" x-text="total"></span><span>laporan</span>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4" :style="'animation-delay:' + (index * 50) + 'ms'">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-800 dark:text-white truncate" x-html="highlightText(report.title)"></h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Jadwal:</span> <span x-text="report.schedule"></span></p>
                        <p class="text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Tersertifikasi:</span> <span x-text="report.total_certified_users"></span></p>
                    </div>
                </div>
                <div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end gap-2">
                    <button @click="deleteReport(report.id)" class="px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-gray-700 rounded-lg transition-colors">Hapus</button>
                </div>
            </div>
        </template>
                ...
                <tbody class="divide-y divide-orange-100 dark:divide-gray-700">
                    <template x-for="(report, index) in reports" :key="report.id">
                        <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150" :style="'animation-delay:' + (index * 50) + 'ms'">
                            <td class="px-6 py-4"><div class="font-semibold text-gray-800 dark:text-white" x-html="highlightText(report.title)"></div></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-text="report.schedule"></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-text="report.total_certified_users"></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <button @click="deleteReport(report.id)" class="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors" title="Hapus"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                </div>
                            </td>
                        </tr>
                    </template>
                    <template x-if="reports.length === 0">
                        <tr><td colspan="4" class="px-6 py-12 text-center text-gray-500"><svg class="w-16 h-16 mx-auto mb-4 text-orange-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><p class="text-lg font-medium">Tidak ada laporan ditemukan</p></td></tr>
                    </template>
                </tbody>
            </table>
        </div>
...
function reportTable() {
    return {
        ...
        columns: [
            ...
            { key: 'total_certified_users', label: 'Total User Tersertifikasi', sortable: false },
        ...
        deleteReport(id) {
            Swal.fire({ title: "Hapus laporan?", text: "Data tidak dapat dikembalikan!", icon: "warning", showCancelButton: true, confirmButtonColor: "#f97316", cancelButtonColor: "#6b7280", confirmButtonText: "Ya, hapus!" }).then((result) => {
                if (result.isConfirmed) {
                    fetch(`/admin/reports/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } })
