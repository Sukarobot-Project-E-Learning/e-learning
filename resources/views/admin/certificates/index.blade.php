...
@section('content')
<div class="container px-6 mx-auto" x-data="certificateTable()">
    <div class="my-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.certificates.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 focus:ring-4 focus:ring-orange-300 transition-all duration-200 transform hover:scale-105">
    ...
    <div class="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-orange-100 dark:border-gray-700">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" @input.debounce.300ms="filterTable()" placeholder="Cari nama program, prefix nomor..." class="w-full pl-10 pr-4 py-2.5 text-sm border-2 border-orange-200 rounded-lg focus:border-orange-500 focus:ring-4 focus:ring-orange-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all duration-200">
        ...
        <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm text-gray-500 dark:text-gray-400">
            <div class="flex items-center gap-1">
                ...
                <span>dari</span><span class="font-semibold text-orange-600" x-text="total"></span><span>template</span>
            </div>
    ...
    <div class="block lg:hidden space-y-3 mb-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4" :style="'animation-delay:' + (index * 50) + 'ms'">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-800 dark:text-white truncate" x-html="highlightText(template.program_name)"></h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            <code class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs" x-text="template.number_prefix"></code>
                        </p>
                        <p class="text-xs text-gray-400 mt-1" x-text="template.created_at"></p>
                    </div>
                    <span :class="template.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'" class="px-2 py-1 text-xs font-bold rounded-full whitespace-nowrap" x-text="template.is_active ? 'Aktif' : 'Nonaktif'"></span>
                </div>
                <div class="mt-3 flex items-center justify-between">
                    <span class="text-xs text-gray-500" x-text="template.certificates_count + ' sertifikat'"></span>
                    <div class="flex items-center gap-2">
                        <a :href="'/admin/certificates/' + template.id + '/edit'" class="px-3 py-1.5 text-sm font-medium text-orange-600 hover:bg-orange-50 dark:hover:bg-gray-700 rounded-lg transition-colors">Edit</a>
                        <button @click="deleteTemplate(template.id)" class="px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-gray-700 rounded-lg transition-colors">Hapus</button>
                    </div>
                </div>
            </div>
        </template>
        <template x-if="templates.length === 0">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
                ...
                <p class="text-gray-500 dark:text-gray-400">Belum ada template sertifikat</p>
                <a href="{{ route('admin.certificates.create') }}" class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Tambah template pertama</a>
            </div>
        </template>
    </div>
                ...
                <tbody class="divide-y divide-orange-100 dark:divide-gray-700">
                    <template x-for="(template, index) in templates" :key="template.id">
                        <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150" :style="'animation-delay:' + (index * 50) + 'ms'">
                            ...
                            <td class="px-6 py-4 text-sm"><code class="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 rounded" x-text="template.number_prefix"></code></td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <span :class="template.certificates_count > 0 ? 'text-green-700 bg-green-100 dark:bg-green-700 dark:text-green-100' : 'text-gray-700 bg-gray-100 dark:bg-gray-700 dark:text-gray-300'" class="px-2 py-1 text-xs font-semibold rounded-full" x-text="template.certificates_count + ' sertifikat'"></span>
                            </td>
                            <td class="px-6 py-4"><span :class="template.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'" class="px-3 py-1 text-xs font-bold rounded-full" x-text="template.is_active ? 'Aktif' : 'Nonaktif'"></span></td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-text="template.created_at"></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <a :href="'/admin/certificates/' + template.id + '/edit'" class="p-2 text-orange-600 hover:bg-orange-100 rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                    <button @click="deleteTemplate(template.id)" class="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                                </div>
                            </td>
                        </tr>
                    </template>
                    <template x-if="templates.length === 0">
                        <tr><td colspan="7" class="px-6 py-12 text-center text-gray-500"><svg class="w-16 h-16 mx-auto mb-4 text-orange-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><p class="text-lg font-medium">Belum ada template sertifikat</p><a href="{{ route('admin.certificates.create') }}" class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Tambah template pertama</a></td></tr>
                    </template>
                </tbody>
            </table>
        </div>
    ...
    <!-- Mobile Pagination -->
    <div class="block lg:hidden bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
        <div class="flex items-center justify-between">
...
function certificateTable() {
    return {
        ...
        columns: [
            ...
            { key: 'template_path', label: 'Blanko', sortable: false },
            { key: 'certificates_count', label: 'Sertifikat Dibuat', sortable: false },
        ...
        fetchData() {
            ...
            fetch('{{ route('admin.certificates.index') }}?' + params.toString(), {
        ...
        deleteTemplate(id) {
            Swal.fire({ title: "Hapus template?", text: "Data tidak dapat dikembalikan!", icon: "warning", showCancelButton: true, confirmButtonColor: "#f97316", cancelButtonColor: "#6b7280", confirmButtonText: "Ya, hapus!" }).then((result) => {
                if (result.isConfirmed) {
                    fetch(`/admin/certificates/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } })
