        ...
        <div class="w-full mb-8 p-6 overflow-hidden rounded-lg shadow-md bg-white dark:bg-gray-800">
            <form id="instructorForm" action="{{ route('admin.instructors.update', $instructor->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <!-- Expertise with Custom Input Option -->
                    <div class="mb-2" x-data="expertiseSelector()">
                        ...
                        <select x-show="!showExpertiseCustom"
                            ...
                            @foreach($expertiseOptions as $option)
                                <option value="{{ $option }}" {{ old('expertise', $instructor->expertise ?? '') == $option ? 'selected' : '' }}>
                                    {{ $option }}
                                </option>
                            @endforeach
                    ...
                    function expertiseSelector() {
                        return {
                            expertise: '{{ old('expertise', $instructor->expertise ?? '') }}',
                            originalExpertise: '{{ old('expertise', $instructor->expertise ?? '') }}',
