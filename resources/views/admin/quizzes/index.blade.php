...
@section('content')
    <div class="container px-6 mx-auto overflow-x-hidden max-w-full" x-data="quizTable()">
            ...
            <div class="flex flex-col md:flex-row gap-4">
                <div class="relative flex-1">
                    ...
                    <input type="text" x-model="search" @input.debounce.300ms="filterTable()"
                        placeholder="Cari judul, instruktur, program..."
                ...
                <select x-model="type" @change="filterTable()"
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm text-gray-500 dark:text-gray-400">
                <div class="flex items-center gap-1">
                    ...
                    <span>dari</span><span class="font-semibold text-orange-600"
                        x-text="total"></span><span>tugas/postest</span>
                </div>
        ...
        <div class="block lg:hidden space-y-3 mb-6">
            <template x-for="(quiz, index) in quizzes" :key="quiz.id">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4"
                    ...
                    <div class="flex items-start gap-3">
                        <div
                            class="w-12 h-12 rounded-lg bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center flex-shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-gray-800 dark:text-white truncate"
                                x-html="highlightText(quiz.title)"></h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 truncate"
                                x-html="highlightText(quiz.instructor || '-')"></p>
                            <p class="text-sm text-gray-500 dark:text-gray-400 truncate"
                                x-html="highlightText(quiz.program || '-')"></p>
                        </div>
                        <span
                            class="px-2 py-1 text-xs font-bold rounded-full bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300 whitespace-nowrap"
                            x-text="quiz.type"></span>
                    </div>
            ...
            <template x-if="quizzes.length === 0">
                <div
                    ...
                    <a href="{{ route('admin.quizzes.create') }}"
                        class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Buat tugas/postest
                        baru</a>
                </div>
            </template>
        </div>
                            ...
                            <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-150"
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-gray-800 dark:text-white"
                                        x-html="highlightText(quiz.title)"></div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                    x-html="highlightText(quiz.instructor || '-')"></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                    x-html="highlightText(quiz.program || '-')"></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                    x-text="quiz.total_questions"></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300"
                                    x-text="quiz.total_responses"></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300" x-text="quiz.created_at">
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <a :href="'/admin/quizzes/' + quiz.id"
                                            class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"
                                            title="Lihat"><svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <template x-if="quizzes.length === 0">
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500"><svg
                                    ...
                                    <p class="text-lg font-medium">Tidak ada tugas/postest ditemukan</p><a
                                        href="{{ route('admin.quizzes.create') }}"
                                        class="mt-2 inline-block text-orange-600 hover:text-orange-700 font-medium">+ Buat
                                        tugas/postest baru</a>
                                </td>
                            </tr>
                        </template>
                    </tbody>
