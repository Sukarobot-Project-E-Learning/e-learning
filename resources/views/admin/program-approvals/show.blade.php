...
@section('content')
    <div class="container px-6 mx-auto">
        ...
        <div class="mb-6">
            @if($approval->status == 'pending')
                <span
                    class="px-4 py-2 text-sm font-semibold text-yellow-700 bg-yellow-100 rounded-lg dark:bg-yellow-700 dark:text-yellow-100">
                    Status: Menunggu Persetujuan
                </span>
            @elseif($approval->status == 'approved')
                <span
                    class="px-4 py-2 text-sm font-semibold text-green-700 bg-green-100 rounded-lg dark:bg-green-700 dark:text-green-100">
                    Status: Disetujui
                </span>
            @else
                <span
                    class="px-4 py-2 text-sm font-semibold text-red-700 bg-red-100 rounded-lg dark:bg-red-700 dark:text-red-100">
                    Status: Ditolak
                </span>
            @endif
        </div>
        ...
        <div class="grid gap-6 mb-8 md:grid-cols-2">
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ ucfirst($approval->type ?? '-') }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                {{ isset($approval->price) ? 'Rp ' . number_format($approval->price, 0, ',', '.') : '-' }}
                            </p>
                        </div>
                        ...
                        @if($approval->image)
                            ...
                            <div>
                                ...
                                <img src="{{ $imageUrl }}" alt="{{ $approval->title }}"
                                    class="mt-2 rounded-lg max-w-full h-auto max-h-64">
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-md bg-white dark:bg-gray-800">
                <div class="p-6">
                    <h3 class="mb-4 text-lg font-semibold text-gray-800 dark:text-gray-200">Informasi Instruktur & Jadwal
                    </h3>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama
                                Instruktur</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $approval->instructor_name ?? '-' }}
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal
                                Diajukan</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                {{ $approval->created_at ? date('d F Y H:i', strtotime($approval->created_at)) : '-' }}</p>
                        </div>
                        ...
                        @if($approval->status == 'approved' && $approval->approved_by_name)
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500">
                                    {{ $approval->approved_at ? date('d F Y H:i', strtotime($approval->approved_at)) : '' }}</p>
                            </div>
                        @endif
        ...
        @if($approval->status == 'pending')
            <div class="flex gap-4 mb-8">
                <form id="approveForm" action="{{ route('admin.program-approvals.approve', $approval->id) }}" method="POST"
                    class="inline-block">
                    @csrf
                ...
                <div class="flex items-center justify-center min-h-screen px-4">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 max-w-md w-full p-6">
                        ...
                        <form action="{{ route('admin.program-approvals.reject', $approval->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Alasan
                                    Penolakan</label>
    ...
    @push('scripts')
        <script>            
            ...
            function initProgramApprovalAlerts() {
                ...
                if (approveBtn && approveForm) {
                    // Remove existing listener if needed (cloning method)
                    ...
                    newApproveBtn.addEventListener('click', function () {
                        Swal.fire({
                            ...
                            text: 'Apakah Anda yakin ingin menyetujui program ini? Program akan langsung dipublikasikan.',
                            icon: 'question',
                ...
                // Reject button opens SweetAlert-styled modal input
