    ...
    @else
        ...
        <li>
            <a href="{{ route('instructor.assignments.index') }}"
                class="group flex items-center px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200
                                          {{ request()->routeIs('instructor.assignments.*') ? $activeClass : $inactiveClass }}">
                <div
                    class="w-9 h-9 rounded-xl flex items-center justify-center mr-3 transition-all duration-200
                                                {{ request()->routeIs('instructor.assignments.*') ? $iconActiveWrapper : $iconInactiveWrapper }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('instructor.assignments.*') ? $iconActive : $iconInactive }}"
                        fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
