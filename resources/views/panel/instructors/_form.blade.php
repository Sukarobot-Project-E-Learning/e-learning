{{-- 
    Reusable Instructor Form Component
    Usage: @include('panel.instructors._form', ['instructor' => $instructor ?? null, 'isEdit' => true/false, 'expertiseOptions' => []])
--}}
...
@php
    ...
    $formAction = $isEdit 
        ? route('admin.instructors.update', $data->id ?? 0)
    ...
    // Avatar URL for edit mode
    $avatarUrl = null;
    if ($isEdit && $data && !empty($data->avatar_url)) {
        $avatarUrl = str_starts_with($data->avatar_url, 'images/') 
            ? asset($data->avatar_url) 
            : asset('storage/' . $data->avatar_url);
    }
@endphp

<div class="instructor-form-container" id="instructorFormContainer">
    <form id="instructorForm" 
          action="{{ $formAction }}" 
          method="POST" 
          enctype="multipart/form-data"
          data-instructor-form>
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        ...
        <div class="p-5 sm:p-8 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="space-y-5">
                ...
                @include('panel.partials.forms.select', [
                    ...
                    'label' => 'Status',
                    'required' => true,
                    'value' => old('status', $data->status ?? 'Aktif'),
                    'options' => [
                        'Aktif' => '✅ Aktif',
                        'Non-Aktif' => '❌ Non-Aktif'
                    ]
        ...
        <div class="p-5 sm:p-8 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="space-y-5">
                ...
                @include('panel.partials.forms.dynamic-select', [
                    ...
                    'label' => 'Keahlian',
                    'required' => true,
                    'value' => old('expertise', $data->expertise ?? ''),
        ...
        <div class="p-5 sm:p-8 border-b border-gray-200 dark:border-gray-700">
            @include('panel.partials.forms.section-header', [
                'title' => 'Keamanan',
                'subtitle' => $isEdit ? 'Ubah password instruktur (opsional)' : 'Password akun instruktur',
                'color' => 'red',
        ...
        <div class="p-5 sm:p-8">
            ...
            @include('panel.partials.forms.image-cropper', [
                ...
                'currentImageAlt' => 'Foto ' . ($data->name ?? 'Instruktur')
            ])
        </div>
    ...
    @include('panel.partials.forms.action-buttons', [
        ...
        'submitText' => $isEdit ? 'Simpan Perubahan' : 'Simpan Instruktur'
    ])
</div>
