<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-none overflow-hidden" id="assignment-app">
    <div class="px-5 sm:px-8 py-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Kelola tugas akhir untuk peserta program
            </p>
        </div>
        <button id="addAssignmentBtn" type="button"
            onclick="openAssignmentModal()"
            class="hidden bg-orange-600 hover:bg-orange-700
    ...
    <div class="p-5 sm:p-8 bg-gray-50 dark:bg-gray-900/50 min-h-[200px]"
        id="assignments-container">
        <!-- Load state -->
        <div class="text-center py-10"
            id="assign-loading-indicator">
            <div class="animate-spin rounded-full h-10 w-10
                border-b-2 border-orange-600 mx-auto mb-3">
            </div>
...
<div id="assignmentModal"
    class="fixed inset-0 z-[100] hidden items-center
        justify-center bg-gray-900/60 backdrop-blur-sm
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl
        ...
        duration-300 mx-auto my-auto"
        id="assignmentModalContent">
        ...
        <div id="assignmentForm">
                ...
                <div>
                    ...
                    <input type="text"
                        ...
                        class="w-full px-4 py-2.5 rounded-xl
                            ...
                            transition-colors"
                        placeholder="Tugas Akhir: Membuat Prototype">
                </div>
                ...
                <div>
                    ...
                    <textarea id="assignment_description"
                        ...
                        class="w-full px-4 py-2.5 rounded-xl
                            ...
                            focus:ring-2 focus:ring-orange-500
                            transition-colors"
                        placeholder="Jelaskan secara detail apa yang harus dilakukan peserta..."
                    ></textarea>
                </div>
            ...
            <div class="px-6 py-4 border-t border-gray-200
                ...
                dark:bg-gray-800/50 flex justify-end gap-3">
                <button type="button"
                ...
                <button type="button"
                    onclick="submitAssignment(event)"
                    class="px-5 py-2 bg-orange-600
...
<script>
    ...
    const assignPrefix = assignIsAdmin
        ? 'admin'
        : 'instructor';
    ...
    function syncAssignHiddenInput() {
        var hInput = document.getElementById(
            'lms_assignment_json'
        );
        if (hInput) {
    ...
    function renderAssignments() {
        ...
        if (assignmentsArray.length === 0) {
            container.innerHTML = ''
                ...
                + 'dark:bg-gray-800 rounded-2xl border '
                + 'border-dashed border-gray-300 '
                + 'dark:border-gray-600">'
                + '<div class="w-16 h-16 bg-purple-50 '
                + 'dark:bg-purple-900/30 rounded-full flex '
                + 'items-center justify-center mx-auto mb-4">'
                + '<i class="fas fa-clipboard-check '
                + 'text-2xl text-purple-500"></i>'
                + '</div>'
                + '<h4 class="text-large font-bold '
                + 'text-gray-800 dark:text-white mb-2">'
                + 'Belum ada Tugas Akhir</h4>'
        ...
        assignmentsArray.forEach(function(asg) {
            var dueDate = asg.due_date
                ? new Date(asg.due_date)
                    .toLocaleDateString('id-ID')
                : 'Tidak Ada Batas Waktu';
            var passingScore = asg.passing_score || 70;
            ...
            html += ''
                ...
                + '<div class="w-12 h-12 bg-purple-50 '
                + 'dark:bg-purple-900/30 rounded-xl flex '
                ...
                + '<i class="fas fa-file-upload mr-1.5 '
                + 'opacity-70"></i> '
                + escapeHtml(asg.allowed_extensions)
                + '</span>'
                ...
                + (!String(asg.id).startsWith('temp_')
                    ? '<a href="/' + assignPrefix + '/programs/' + assignProgramId + '/lms/submissions" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors">'
                ...
                + asg.id + '\')" '
                + 'class="p-2 text-gray-500 opacity-100 '
                ...
                + 'dark:bg-gray-900/50 rounded-xl '
    ...
    function openAssignmentModal(id) {
        ...
        if (!id) {
            document.getElementById('assignment_title')
                .value = '';
            ...
            ).value = 'pdf, zip, rar, doc, docx, png, jpg';
    ...
    function editAssignment(id) {
        var asg = assignmentsArray.find(function(a) {
        ...
        document.getElementById(
            'assignment_allowed_extensions'
        ).value = asg.allowed_extensions || '';
        document.getElementById('assignment_due_date')
            .value = asg.due_date
                ? asg.due_date.split(' ')[0]
                : '';
        document.getElementById(
            'assignment_passing_score'
        ).value = asg.passing_score || 70;
    ...
    function submitAssignment(e) {
        ...
        var extensions = document.getElementById(
            'assignment_allowed_extensions'
        ).value.trim();
        var passingScore = parseInt(
            document.getElementById(
                'assignment_passing_score'
            ).value, 10
        ) || 70;
        ...
        if (passingScore < 0 || passingScore > 100) {
            alert('Nilai kelulusan harus 0-100');
            return;
        }
        ...
        var payload = {
            ...
            due_date: document.getElementById(
                'assignment_due_date'
            ).value || null,
        ...
        if (id) {
            var idx = assignmentsArray.findIndex(
                function(a) {
                    return String(a.id) === String(id);
                }
            );
    ...
    function deleteAssignment(id) {
        Swal.fire({
            title: 'Hapus Tugas Akhir?',
            text: 'Data yang dihapus tidak dapat '
                + 'dikembalikan!',
            icon: 'warning',
