        ...
        <div class="w-full mb-8 overflow-hidden rounded-lg shadow-md bg-white dark:bg-gray-800">
            <div class="px-6 py-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $quiz->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ ucfirst($quiz->status) }}
                            </span>
                        </div>
                    </div>
                    <div class="mt-4">
                        ...
                        <p class="mt-1 text-gray-800 dark:text-gray-200 whitespace-pre-line">{{ $quiz->description ?? '-' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Daftar Pertanyaan
                        ({{ count($questions) }})</h3>
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-2">
                                            ...
                                            <span
                                                class="text-xs font-medium px-2 py-0.5 rounded bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                                        ...
                                        <p class="text-gray-800 dark:text-gray-200 font-medium mb-3">{{ $question->question }}
                                        </p>
                                            ...
                                            <div class="ml-8 space-y-2">
                                                @if($question->options)
                                                    @foreach($question->options as $optIndex => $option)
                                                        <div class="flex items-center gap-2">
                                                            <div
                                                                class="w-4 h-4 border border-gray-300 rounded-full flex items-center justify-center {{ $question->correct_answer == $option ? 'bg-green-500 border-green-500' : '' }}">
                                                            ...
                                                            <span
                                                                class="text-sm {{ $question->correct_answer == $option ? 'text-green-700 font-medium dark:text-green-400' : 'text-gray-600 dark:text-gray-400' }}">
                                                                {{ $option }}
                                                            </span>
                                                        </div>
                                                    @endforeach
