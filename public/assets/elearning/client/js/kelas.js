document.addEventListener("DOMContentLoaded", () => {
    ...
    let activeCategory =
        (
            document.querySelector(".filter-btn.bg-orange-500") ||
            document.querySelector(".filter-btn.active")
        )?.dataset.filter || "all";
    let sortOrder =
        document.querySelector(".sort-btn.active")?.dataset.sort || "newest";
    let searchQuery = "";
    ...
    function applyAll() {
        ...
        filtered.sort((a, b) => {
            const da = a.dataset.date
                ...
                : parseInt(a.dataset.origIndex || 0, 10);
    ...
    filterButtons.forEach((btn) => {
        btn.addEventListener("click", (e) => {
            ...
            applyAll();
            try {
                window.location.hash = `filter=${encodeURIComponent(
                    activeCategory
                )}`;
            } catch (err) {
                /* ignore */
    ...
    const activeSort =
        document.querySelector(".sort-btn.active") ||
        document.querySelector('.sort-btn[data-sort="newest"]');
    if (activeSort) {
        activeSort.classList.add("active");
        sortOrder = activeSort.dataset.sort || "newest";
    }
