/**
 * Instructor Header Component Script
 * Handles profile dropdowns, navigation dropdowns (Kompetisi, Tentang), mobile menu toggle, and keyboard navigation.
 * Instructor-specific version with client navigation links.
 */

document.addEventListener("DOMContentLoaded", function () {
    console.log("[Instructor Header] Script initialized");
    ...
    const CONFIG = {
        ...
        // Navigation dropdowns (instructor only)
        kompetisiDropdownId: 'kompetisi-dropdown',
        kompetisiMenuId: 'kompetisi-menu',
        tentangDropdownId: 'tentang-dropdown',
        tentangMenuId: 'tentang-menu',
        // Mobile sidebar
    ...
    const kompetisiDropdown = document.getElementById(CONFIG.kompetisiDropdownId);
    const kompetisiMenu = document.getElementById(CONFIG.kompetisiMenuId);
    const tentangDropdown = document.getElementById(CONFIG.tentangDropdownId);
    const tentangMenu = document.getElementById(CONFIG.tentangMenuId);
    const mobileSidebarBtn = document.getElementById(CONFIG.mobileSidebarBtnId);

    // Debug: Log element detection
    console.log("[Instructor Header] Elements found:", {
        header: !!header,
        ...
        kompetisiDropdown: !!kompetisiDropdown,
        kompetisiMenu: !!kompetisiMenu,
        tentangDropdown: !!tentangDropdown,
    ...
    function closeAllDropdowns() {
        ...
        // Kompetisi
        if (state.kompetisiOpen) {
            state.kompetisiOpen = false;
            hideDropdown(kompetisiMenu);
            toggleChevron(kompetisiDropdown, false);
        }
        // Tentang
        if (state.tentangOpen) {
            state.tentangOpen = false;
            hideDropdown(tentangMenu);
            toggleChevron(tentangDropdown, false);
        }
    }
    ...
    if (desktopProfileTrigger && desktopProfileMenu) {
        ...
        if (profileContainer) {
            console.log("[Instructor Header] Desktop profile hover events attached");
            ...
            console.warn("[Instructor Header] Desktop profile container (.relative) not found");
        }
    } else {
        console.warn("[Instructor Header] Desktop profile dropdown elements not found");
    }

    // === Mobile Profile Dropdown (Click-based) ===
    if (mobileProfileTrigger && mobileProfileMenu) {
        console.log("[Instructor Header] Mobile profile click events attached");
        ...
        console.warn("[Instructor Header] Mobile profile dropdown elements not found");
    }

    // === Kompetisi Dropdown (Hover-based, Desktop only) ===
    if (kompetisiDropdown && kompetisiMenu) {
        const kompetisiContainer = kompetisiDropdown.closest('.relative');

        if (kompetisiContainer) {
            console.log("[Instructor Header] Kompetisi dropdown hover events attached");

            kompetisiContainer.addEventListener('mouseenter', function () {
                clearHoverTimer('kompetisi');
                state.kompetisiOpen = true;
                showDropdown(kompetisiMenu);
                toggleChevron(kompetisiDropdown, true);
            });

            kompetisiContainer.addEventListener('mouseleave', function () {
                state.hoverTimers['kompetisi'] = setTimeout(() => {
                    state.kompetisiOpen = false;
                    hideDropdown(kompetisiMenu);
                    toggleChevron(kompetisiDropdown, false);
                }, CONFIG.closeDelay);
    ...
    // === Tentang Dropdown (Hover-based, Desktop only) ===
    if (tentangDropdown && tentangMenu) {
        const tentangContainer = tentangDropdown.closest('.relative');

        if (tentangContainer) {
            console.log("[Instructor Header] Tentang dropdown hover events attached");

            tentangContainer.addEventListener('mouseenter', function () {
                clearHoverTimer('tentang');
                state.tentangOpen = true;
                showDropdown(tentangMenu);
                toggleChevron(tentangDropdown, true);
            });

            tentangContainer.addEventListener('mouseleave', function () {
                state.hoverTimers['tentang'] = setTimeout(() => {
                    state.tentangOpen = false;
                    hideDropdown(tentangMenu);
                    toggleChevron(tentangDropdown, false);
                }, CONFIG.closeDelay);
    ...
    if (mobileSidebarBtn) {
        console.log("[Instructor Header] Mobile sidebar button events attached");
        ...
        console.warn("[Instructor Header] Mobile sidebar button not found");
    }
    ...
    console.log("[Instructor Header] Initialization complete");
});
